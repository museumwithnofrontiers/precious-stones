...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('preciousStones.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('preciousStones.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('preciousStones.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
