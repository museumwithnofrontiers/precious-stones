...
describe('website smoke test', () => {
  ...
  it('renders the about page on the composed text page view', async () => {
    ...
    expect(host.textContent).toContain("Precious Stones")
  ...
  it('colours and names the source-database chip from the manifest projects section', async () => {
    const { app, host } = await mountSite('#/item/7fd24a74-b039-5ba4-b6a9-6fe4522c585f')
  ...
  it('shows the explore-partner notice only for the project dataset.config.js lists', async () => {
    const epm = await mountSite('#/item/6157df5e-6cc0-55c3-aebb-5ece5ba7125c')
    ...
    const isl = await mountSite('#/item/7fd24a74-b039-5ba4-b6a9-6fe4522c585f')
  ...
  it('renders the related-database and artistic-introduction links from the manifest', async () => {
    const [items] = await loadEntities(['items'])
    const item = items.find((i) => i.id === '7fd24a74-b039-5ba4-b6a9-6fe4522c585f')
  ...
  it('shows the source project on a collection-results tile, from the manifest', async () => {
    ...
    const item = items.find((i) => i.project_id === islProjectId) ?? { internal_name: "Gold necklace." }
  ...
  it('renders the layout glossary tool and dynasty popouts on the item sheet', async () => {
    ...
    const { app, host } = await mountSite('#/item/32fdd8a2-bebf-5ad2-bcf2-45ef21b39556')
  ...
  it('renders the partners list on the composed partner-list view', async () => {
    ...
    expect(host.textContent).toContain("United Kingdom")
    expect(host.textContent).toContain('The British Museum')
  ...
  it('renders a partner profile on the composed record view', async () => {
    const { app, host } = await mountSite('#/partner/af01a84d-b174-5718-b9a7-b741c8957086')
    ...
    await vi.waitFor(() => expect(host.textContent).toContain('The British Museum'), { timeout: 20000 })
    await vi.waitFor(() => expect(host.textContent).toContain("London"), { timeout: 20000 })
  ...
  it("renders a partner's objects on the composed grid results view", async () => {
    const { app, host } = await mountSite('#/partner/af01a84d-b174-5718-b9a7-b741c8957086/objects')
  ...
  it('renders the shared texts and its own over them', async () => {
    ...
    expect(checkTextsRendered(host, {
      namespaces: ['preciousStones', 'core', 'layout', 'catalogue', 'record', 'sheet', 'timeline', 'partner', 'gallery'],
    })).toEqual([])
